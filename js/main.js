...
function build() {
        ...
        for (var j = 0; j < i; j++){
            ...
            if (!((forward == 0 && backward == 0)||(i == j))){
                if (forward > 0 && backward > 0){
                    g.addEdge("id" + i, "id" + j, {directed: false, stroke: "#fff", fill: "#5a5", label: ""+forward});
                } else if (forward > backward){
                    g.addEdge("id" + i, "id" + j, {directed: true, stroke: "#fff", fill: "#5a5", label: ""+forward});
                } else {
                    g.addEdge("id" + j, "id" + i, {directed: true, stroke: "#fff", fill: "#5a5", label: ""+backward});
                }
            }
        }
    }
    ...
    if (from > matrix.length || to > matrix.length){
        alert ("Ошибка во вводе: неверно указаны пункты для нахождения пути");
        return false;
    }
    ...
    let answer = await fetch('api.php',{
      ...
      body: 'matrix=' + matrixToSend + "&from=" + from + "&to=" + to + "&size=" + matrix.length
    }),
